...
/**
 * ESTRATEGIA: JOINED TABLE (JT)
 * - Tabla base con propiedades comunes
 * - Tablas derivadas con propiedades específicas
 * - Relación 1:1 entre tablas base y derivadas
 * - Consultas requieren JOIN pero esquema normalizado
 */
...
export class CarDetail {
  ...
  isLuxury(): boolean {
    return this.vehicle.price >= 50000;
  }
...
// Tabla específica para motocicletas
@Entity("motorcycle_details")
...
// Clases de conveniencia que combinan base + detalles
export class Car {
  vehicle: Vehicle;
  detail: CarDetail;

  constructor(vehicle: Vehicle, detail: CarDetail) {
  ...
  getDisplayInfo(): string {
    return `${this.vehicle.getBasicInfo()} - ${this.detail.doors}-door ${
      this.detail.transmission
    } (${this.detail.mpg} MPG)`;
  }

  getSpecifications(): string {
    return this.detail.getSpecifications();
  }

  isEconomical(): boolean {
    return this.detail.isEconomical();
  }
...
export class Motorcycle {
  ...
  constructor(vehicle: Vehicle, detail: MotorcycleDetail) {
    this.vehicle = vehicle;
    this.detail = detail;
  }
  ...
  getDisplayInfo(): string {
    return `${this.vehicle.getBasicInfo()} - ${this.detail.engineSize}cc ${
      this.detail.motorcycleType
    } (${this.detail.topSpeed} mph)`;
  }
  ...
  isHighPerformance(): boolean {
    return this.detail.isHighPerformance();
  }

  isBeginnerFriendly(): boolean {
    return this.detail.isBeginnerFriendly();
  }
}
...
export class Truck {
  ...
  constructor(vehicle: Vehicle, detail: TruckDetail) {
    this.vehicle = vehicle;
    this.detail = detail;
  }
  ...
  getDisplayInfo(): string {
    return `${this.vehicle.getBasicInfo()} - ${
      this.detail.cabType
    } cab, ${this.detail.driveType.toUpperCase()} (${this.detail.payloadCapacity.toLocaleString()} lbs payload)`;
  }
  ...
  isHeavyDuty(): boolean {
    return this.detail.isHeavyDuty();
  }

  canTow(weight: number): boolean {
    return this.detail.canTow(weight);
  }
}
